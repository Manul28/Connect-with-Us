...
class supplierClass:
    def __init__(self, root):
       ...
       # self.root.place(x=250, y=20, width=600, height=70)
        ...
        txt_search = Entry(self.root, textvariable=self.var_searchtxt, font=(
            "goudy old style", 15), bg="lightyellow").place(x=800, y=80, width=160)
        btn_search = Button(self.root, text="Search", command=self.search, font=("goudy old style", 15),
                            bg="#4caf50", fg="white", cursor="hand2").place(x=980, y=79, width=100, height=28)
        ...
        # content
        # row-1
        lb1_supplier_invoice = Label(self.root, text="Invoice No.", font=(
        ...
        # row-4
        lb1_desc = Label(self.root, text="Description", font=(
        ...
        btn_clear = Button(self.root, text="Clear", command=self.clear, font=("goudy old style", 15), bg="#607d8b",
                           fg="white", cursor="hand2").place(x=540, y=370, width=110, height=35)

        # Employee Details

        emp_frame = Frame(self.root, bd=3, relief=RIDGE)
        emp_frame.place(x=660, y=120, width=420, height=350)
        ...
        try:
            if self.var_sup_invoice.get() == "":
                messagebox.showerror(
                    "Error", f"Invoice must be required:", parent=self.root)
            else:
                ...
                if row != None:
                    messagebox.showerror(
                        "Error", "Invoice no. already assigned,try any different", parent=self.root)
                else:
    ...
    def update(self):
        ...
        try:
            if self.var_sup_invoice.get() == "":
                messagebox.showerror(
                    "Error", f"supplier invoice must be required:", parent=self.root)
            else:
                cur.execute("Select *from supplier where invoice=?",
                            (self.var_sup_invoice.get(),))
                row = cur.fetchone()
    ...
    def delete(self):
        ...
        try:
            if self.var_sup_invoice.get() == "":
                ...
                if row == None:
                    messagebox.showerror(
                        "Error", "Invoice ID", parent=self.root)
                else:
                    op = messagebox.askyesno(
                        "Confirm", "Do you really want to delete this supplier information?", parent=self.root)
                    if op == True:
                        cur.execute("delete from supplier where invoice=?",
                                    (self.var_sup_invoice.get(),))
                        con.commit()
                        messagebox.showinfo(
                            "Delete", "Supplier Successfully deleted", parent=self.root)
                        self.clear()
